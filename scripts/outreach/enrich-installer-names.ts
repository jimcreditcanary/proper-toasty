...
/**
 * Normalise a company number for Companies House lookup.
 *
 ...
 * SO* Scottish LLPs, NC* NI LLPs etc.) are uppercased and left at
 * their natural length — those formats already include the prefix.
 *
 * Examples:
...
> {
  // UK Companies House numbers are 8 characters. The MCS-scraped
  // values in `installers.company_number` often arrive as 7-digit
  // strings (leading zero dropped during whatever spreadsheet step
  // produced the import). The CH API returns a 200 OK with an empty
  // items[] for malformed numbers instead of a 404 — pure cache
  // poison. Numbers with a letter prefix (SC*, NI*, OC*) stay as-is.
  const cleaned = normaliseCompanyNumber(companyNumber);
